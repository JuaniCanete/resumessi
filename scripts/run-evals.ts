...
import { validateSchema, checkFaithfulness, checkHallucination, checkRelevance, checkCompleteness, SemanticMetrics } from './eval-metrics.js';
...
const CleanJdOutputSchema = z.object({
  cleanedText: z.string().min(10),
}).passthrough();

const ScraperParametersOutputSchema = z.object({
  role: z.string().min(1),
...
const ScraperSummarizeOutputSchema = z.object({
...
async function evaluatePromptType(
  ...
  for (const golden of goldens) {
    ...
    try {
      ...
      const semanticPassed = context.mode === 'mock' 
        ? true 
      ...
      const status = passed ? '✓' : '✗';
      console.log(`  ${status} ${id}: schema=${schemaResult.valid ? 'pass' : 'fail'} faithfulness=${semanticMetrics.faithfulness.score.toFixed(2)} hallucination=${semanticMetrics.hallucination.score.toFixed(2)} relevance=${semanticMetrics.relevance.score.toFixed(2)} completeness=${semanticMetrics.completeness.score.toFixed(2)}`);
      ...
      if (regression && regression.regressions.length > 0) {
        for (const reg of regression.regressions) {
          console.log(`    🔴 REGRESSION: ${reg.metric} ${reg.baselineScore.toFixed(2)} → ${reg.currentScore.toFixed(2)} (${reg.severity})`);
        }
      }
      ...
      results.push({
        ...
        semanticMetrics: { faithfulness: { score: 0, details: [] }, hallucination: { score: 0, details: [] }, relevance: { score: 0, details: [] }, completeness: { score: 0, details: [] } },
        runResult: { output: null, usage: { promptTokens: 0, completionTokens: 0, totalTokens: 0 }, latencyMs: 0, provider: 'error', model: 'error', mode: context.mode },
        passed: false,
      });
    }
  }
...
async function runEvals(context: EvalContext): Promise<boolean> {
  ...
  const promptTypesToEval = context.promptTypes.length > 0 
    ? context.promptTypes 
    : listAvailablePromptTypes();
