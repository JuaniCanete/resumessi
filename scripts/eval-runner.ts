...
const MODEL_PRICING: Record<string, { input: number; output: number }> = {
  ...
  'command-r-plus': { input: 0.50 / 1e6, output: 1.50 / 1e6 },
  'mistral-large': { input: 2.00 / 1e6, output: 6.00 / 1e6 },
  'llama-3.1-70b': { input: 0.59 / 1e6, output: 0.79 / 1e6 },
};

const MAX_COST_PER_RUN_USD = 0.50;
const MAX_TOTAL_TOKENS = 50000;

function estimateCost(provider: string, model: string, usage: { promptTokens: number; completionTokens: number }): number {
  const key = model.toLowerCase();
  const pricing = MODEL_PRICING[key] || MODEL_PRICING[Object.keys(MODEL_PRICING).find(k => key.includes(k)) || 'gpt-4o-mini'];
...
export async function runEvalInference<T>(
  ...
  const cost = estimateCost(result.provider || 'unknown', result.model || 'unknown', { promptTokens, completionTokens });
  ...
  if (mode === 'record') {
    ...
    writeFileSync(join(recordDir, filename), JSON.stringify({
      systemPrompt,
      userPrompt,
      output: parsedOutput,
