...
import { getVariantStyles, showConfirmModal, showToast, confirmDelete, confirmUnsave, showApplyModal, __resetModalState } from '../../public/utils/modal.ts';
...
test('showToast - success variant renders with correct colors', () => {
	...
	assert.ok(toast.style.background.includes('rgba(16, 185, 129') || toast.style.background.includes('rgb(16, 185, 129'));
...
test('showToast - warning variant renders with correct colors', () => {
	...
	assert.ok(toast.style.background.includes('rgba(245, 158, 11') || toast.style.background.includes('rgb(245, 158, 11'));
});
...
test('showConfirmModal - variant danger renders danger styles', () => {
	...
	assert.ok(confirmBtn.style.background.includes('#ef4444') || confirmBtn.style.background.includes('rgb(239, 68, 68)'));
});

test('confirmDelete - builds correct modal options', () => {
	let confirmCalled = false;
	confirmDelete('Test Item', () => { confirmCalled = true; });
...
test('confirmUnsave - builds correct modal options', () => {
	let confirmCalled = false;
	confirmUnsave('Saved Job', () => { confirmCalled = true; });
...
test('showApplyModal - confirms with valid input', async () => {
	...
	showApplyModal({
		item: { title: 'Test', company: 'Test' },
		onConfirm: (name: string) => { confirmCalled = true; confirmedName = name; },
	});
