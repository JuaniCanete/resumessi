...
after(() => {
	...
	if (fs.existsSync(TEST_DB_DIR)) {
		fs.readdirSync(TEST_DB_DIR).forEach(f => {
			if (f.startsWith('jobdata-unit-test')) {
				try { rmSync(join(TEST_DB_DIR, f), { force: true }); } catch {}
			}
		});
	}
});
...
import {
	...
	closeDb,
	setDbPathForTesting,
} from '../../src/storage/jobDataSqlite.ts';
...
function makeRun(source: 'linkedin' | 'google' | 'remoterocketship', jobs: ReturnType<typeof makeJob>[]): {
	timestamp: string;
...
test('setScrapingRun - UPSERT preserves removed/saved/applied flags from existing row in scraping_results', async () => {
	const { dbPath, cleanup } = getTestDb();
	const job = makeJob();
...
test('updateDashboardJob - matches by id (jobId) when only id is provided', async () => {
	...
	await applyToJob(job, job.source);
 	...
 	await updateDashboardJob(undefined, { status: 'Applied', column: 'applied' }, found.id);
	
	const dashboard2 = await getJobDashboard();
	const found2 = dashboard2.find(j => j.url === job.url);
	assert.ok(found2);
	assert.equal(found2.status, 'Applied');
...
test('updateDashboardJob - uses AND clause when both url and id provided - requires both to match', async () => {
	...
	await applyToJob(job, job.source);
 	...
 	await updateDashboardJob(job.url, { status: 'Wrong' }, 999999);
	
	const dashboard2 = await getJobDashboard();
	const found2 = dashboard2.find(j => j.url === job.url);
	assert.ok(found2);
	assert.notEqual(found2.status, 'Wrong');
