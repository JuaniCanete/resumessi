...
test('card shows source badge', async ({ findJobPage }) => {
	...
	await expect(findJobPage.getCardsInColumn('applied').first().locator('.board-card-source')).toHaveText('LinkedIn');
	await expect(findJobPage.getCardsInColumn('screening').first().locator('.board-card-source')).toHaveText('Google');
