 ...
 * 
 * Config:
 *   Reads provider keys from .env via src/providers.js
...
// ── Config Loading ─────────────────────────────────────────────────
function loadConfig() {
    const providerLib = require('./providers.js');
    const env = providerLib.getProviderConfigFromEnv ? 
        providerLib.getProviderConfigFromEnv() : 
        loadEnvDirectly();
    
    const configured = providerLib.getProviderConfig(env).configured;
    if (configured.length === 0) {
        console.error('No AI providers configured. Run: node setup.js');
        process.exit(1);
    }
...
// ── AI API Call ──────────────────────────────────────────────────
async function callAI(system, prompt, env) {
    const router = require('./router.js');
    
    const result = await router.runInference(system, prompt, {}, env);
    let raw = result.text;
...
async function main() {
    ...
    try {
        const result = await callAI(RESUME_PROMPT, RESUME_PROMPT + '\n\n=== RAW RESUME INPUT ===\n' + userInput, env);
