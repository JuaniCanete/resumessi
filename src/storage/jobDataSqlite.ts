...
export function getDb(): Database.Database {
	...
	if (process.env.NODE_ENV !== 'test' && (DB_PATH.includes('-test') || DB_PATH.includes('/test/'))) {
		console.warn(
			`[Storage] WARNING: Using test database path: ${DB_PATH}. Set NODE_ENV=test to suppress this warning.`
		);
	}
	...
	// Migration: add cleaned column if missing
	for (const table of ['scraping_results', 'saved_jobs', 'job_dashboard']) {
		try {
			db.exec(`ALTER TABLE ${table} ADD COLUMN cleaned INTEGER DEFAULT 0`);
		} catch {
			// Column already exists
		}
	}
...
export async function getScrapingRun(
	...
	const runRow = database.prepare('SELECT * FROM scraper_runs WHERE source = ?').get(source) as
	...
	const removedCountRow = database
		.prepare('SELECT COUNT(*) as c FROM scraping_results WHERE source = ? AND removed = 1')
		.get(source) as { c: number } | undefined;
	const removedCount = removedCountRow?.c || 0;
...
export async function setScrapingRun(
	...
	// Re-query database to get results with preserved flags (removed, saved, applied)
	const actualResults = database
		.prepare('SELECT * FROM scraping_results WHERE source = ? AND removed = 0')
		.all(source) as Record<string, unknown>[];
	const resultsWithFlags = actualResults.map(parseRowFromSqlite);

	// Get removed count for this source
	const removedCountRow = database
		.prepare('SELECT COUNT(*) as c FROM scraping_results WHERE source = ? AND removed = 1')
		.get(source) as { c: number } | undefined;
	const removedCount = removedCountRow?.c || 0;
	...
	setCachedRun(source, {
		...
		source,
		query: run.query,
		totalResults: resultsWithFlags.length,
...
export async function loadJobData(): Promise<JobData> {
	...
	const googleResults = database
		.prepare("SELECT * FROM scraping_results WHERE source = 'google' AND removed = 0")
		.all() as Record<string, unknown>[];
	const linkedinSaved = database.prepare("SELECT * FROM saved_jobs WHERE source = 'linkedin'").all() as Record<
		string,
		unknown
	>[];
	const googleSaved = database.prepare("SELECT * FROM saved_jobs WHERE source = 'google'").all() as Record<
		string,
		unknown
	>[];
...
export async function saveJobFromScraping(result: ScraperResult, source: 'linkedin' | 'google'): Promise<void> {
	...
	const db = getDb();
	const existing = db.prepare('SELECT * FROM saved_jobs WHERE url = ? AND source = ?').get(result.url, source) as
		Record<string, unknown> | undefined;
	if (existing) {
...
export async function unsaveJob(source: 'linkedin' | 'google', url: string): Promise<void> {
	...
	const dbTransaction = database.transaction(() => {
		...
		database
			.prepare('UPDATE scraping_results SET saved = 0, savedAt = NULL WHERE url = ? AND source = ?')
			.run(url, source);
	});
	dbTransaction();
	clearCachedRun();
...
export async function applyToJob(
	...
	if (result.url) {
		const urlExisting = database
			.prepare('SELECT COUNT(*) as count FROM job_dashboard WHERE url = ?')
			.get(result.url) as { count: number };
		if (urlExisting.count > 0) {
			throw new Error('DUPLICATE_URL');
		}
	} else {
		const titleLower = title.toLowerCase();
		const titleExisting = database
			.prepare('SELECT COUNT(*) as count FROM job_dashboard WHERE LOWER(title) = ?')
			.get(titleLower) as { count: number };
    ...
    `);
		...
		if (result.saved) {
			database
				.prepare('UPDATE saved_jobs SET applied = 1, appliedAt = ?, status = ? WHERE url = ? AND source = ?')
				.run(dashboardJob.appliedAt, 'No News', result.url, source);
		} else {
			database
				.prepare('UPDATE scraping_results SET applied = 1, appliedAt = ?, status = ? WHERE url = ? AND source = ?')
				.run(dashboardJob.appliedAt, 'No News', result.url, source);
		}
	});
	dbTransaction();
	...
	if (changed) {
		const dbTransaction = database.transaction(() => {
			for (const result of results) {
				...
				if (result.url) {
					database
						.prepare("UPDATE job_dashboard SET jobId = ? WHERE url = ? AND (jobId IS NULL OR jobId = '')")
						.run(result.id, result.url);
					database
						.prepare("UPDATE job_dashboard SET column = ? WHERE url = ? AND (column IS NULL OR column = '')")
						.run(result.column, result.url);
				} else if (rowid) {
					database
						.prepare("UPDATE job_dashboard SET jobId = ? WHERE rowid = ? AND (jobId IS NULL OR jobId = '')")
						.run(result.id, rowid);
					database
						.prepare("UPDATE job_dashboard SET column = ? WHERE rowid = ? AND (column IS NULL OR column = '')")
...
export async function updateDashboardJob(
	...
	if (url && id) {
		const stmt = database.prepare(`UPDATE job_dashboard SET ${setParts.join(', ')} WHERE url = ? AND jobId = ?`);
		stmt.run(...values, url, id);
	} else if (url) {
		const stmt = database.prepare(`UPDATE job_dashboard SET ${setParts.join(', ')} WHERE url = ?`);
		stmt.run(...values, url);
	} else if (id) {
		const stmt = database.prepare(`UPDATE job_dashboard SET ${setParts.join(', ')} WHERE jobId = ?`);
		stmt.run(...values, id);
	}
}
...
export async function updateJobDescription(url: string, jobDescription: string): Promise<void> {
	...
	const dbTransaction = database.transaction(() => {
		...
		database.prepare('UPDATE saved_jobs SET jobDescription = ? WHERE url = ?').run(jobDescription, url);
		database.prepare('UPDATE job_dashboard SET jobDescription = ? WHERE url = ?').run(jobDescription, url);
	});
	dbTransaction();
	clearCachedRun();
}
